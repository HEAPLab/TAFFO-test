...
int Image::loadRgbImage(std::string filename)
{
	...
	for (int h = 0 ; h < this->height ; h++)
	{
		...
		{
			int index = w * 3 ;
			#pragma taffo r loadRgbImage "scalar(range(0,65535))"
			#pragma taffo g loadRgbImage "scalar(range(0,65535))"
			#pragma taffo b loadRgbImage "scalar(range(0,65535))"
...
void Image::makeGrayscale()
{
	...
	float  rC = 0.30 / 65536.0 ;
	float  gC = 0.59 / 65536.0 ;
	float  bC = 0.11 / 65536.0 ;
