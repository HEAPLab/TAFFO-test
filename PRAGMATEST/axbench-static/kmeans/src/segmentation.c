...
int initClusters(Clusters* clusters,int k, float  scale) {
	...
	DO_PRAGMA(taffo clusters initClusters ANNOTATION_CLUSTER)
	#pragma taffo scale initClusters "scalar()"

	int i;
	#pragma taffo x initClusters "scalar()"
	float x;
...
void freeClusters(Clusters* clusters) {
	DO_PRAGMA(taffo clusters freeClusters ANNOTATION_CLUSTER)
	if (clusters != NULL)
		free(clusters->centroids);
}

void segmentImage(RgbImage* image,
		  Clusters* clusters, int n) {
	//function parameters pragmas
	DO_PRAGMA(taffo image segmentImage ANNOTATION_RGBIMAGE)
	DO_PRAGMA(taffo clusters segmentImage ANNOTATION_CLUSTER)
	int i;
	int x, y;
	int c;
