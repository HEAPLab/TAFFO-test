...
int main(int argc, const char* argv[])
{
	...
	#pragma taffo t1t2xy main "target('t1t2xy') scalar()"
	...
	for (int i=0; i<n*2*2; i+=2*2) {
		DO_PRAGMA(taffo theta1 main "scalar(" ANNOTATION_RANGE " error(1e-8) disabled)")
		DO_PRAGMA(taffo theta2 main "scalar(" ANNOTATION_RANGE " error(1e-8) disabled)")
		float theta1, theta2;
