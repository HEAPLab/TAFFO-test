; ModuleID = './test3.out.1.taffotmp.ll'
...
; Function Attrs: noinline nounwind uwtable
define dso_local float @test(i32 %p1, i32 %p2, i32 %p3, float %num1, float %num2, float %num3) #0 {
entry:
  ...
  %phi1 = bitcast float* %phi to i8*
  call void @llvm.var.annotation(i8* %phi1, i8* getelementptr inbounds ([19 x i8], [19 x i8]* @.str, i32 0, i32 0), i8* getelementptr inbounds ([10 x i8], [10 x i8]* @.str.1, i32 0, i32 0), i32 18)
...
if.then:                                          ; preds = %entry
  ...
  store float %1, float* %phi, align 4
  br label %if.end

if.else:                                          ; preds = %entry
  store float 1.500000e+00, float* %phi, align 4
  br label %if.end

if.end:                                           ; preds = %if.else, %if.then
  %call = call float @random()
  %2 = load float, float* %phi, align 4
  %mul = fmul float %2, %call
...
if.then3:                                         ; preds = %if.end
  ...
  %5 = load float, float* %phi, align 4
  %add = fadd float %5, %4
  store float %add, float* %phi, align 4
  %call4 = call float @random()
  %6 = load float, float* %phi, align 4
  %mul5 = fmul float %6, %call4
  store float %mul5, float* %phi, align 4
  br label %if.end8

if.else6:                                         ; preds = %if.end
  %call7 = call float @random()
  %7 = load float, float* %phi, align 4
  %div = fdiv float %7, %call7
  store float %div, float* %phi, align 4
  br label %if.end8
...
; Function Attrs: noinline nounwind uwtable
define dso_local float @test2(i32 %p1) #0 {
entry:
  %p1.addr = alloca i32, align 4
  %phi = alloca float, align 4
  store i32 %p1, i32* %p1.addr, align 4
  %phi1 = bitcast float* %phi to i8*
  call void @llvm.var.annotation(i8* %phi1, i8* getelementptr inbounds ([19 x i8], [19 x i8]* @.str, i32 0, i32 0), i8* getelementptr inbounds ([10 x i8], [10 x i8]* @.str.1, i32 0, i32 0), i32 37)
  store float 0.000000e+00, float* %phi, align 4
  %0 = load i32, i32* %p1.addr, align 4
  %tobool = icmp ne i32 %0, 0
  br i1 %tobool, label %if.then, label %if.else
...
if.then:                                          ; preds = %entry
  %call = call float @random()
  %1 = load float, float* %phi, align 4
...
if.else:                                          ; preds = %entry
  ...
  %add2 = fadd double %conv, 2.000000e+00
  %conv3 = fptrunc double %add2 to float
  store float %conv3, float* %phi, align 4
  br label %if.end

if.end:                                           ; preds = %if.else, %if.then
  %call4 = call float @random()
  %3 = load float, float* %phi, align 4
  %add5 = fadd float %3, %call4
  store float %add5, float* %phi, align 4
