...
public:
	...
	float getPixel_r(int x, int y) __attribute((always_inline)) {
	  DO_PRAGMA(taffo pixels getPixel_r ANNOTATION_PIXEL)
	  float *pixels = ((float*)_pixels);
	  return pixels[y * (IMAGE_WIDTH(this) * 3) + x * 3 + 0];
	}
	inline float getPixel_g(int x, int y)  __attribute((always_inline)) {
	  DO_PRAGMA(taffo pixels getPixel_g ANNOTATION_PIXEL)
	  float *pixels = ((float*)_pixels);
	  return pixels[y * (IMAGE_WIDTH(this) * 3) + x * 3 + 1];
	}
	inline float getPixel_b(int x, int y)  __attribute((always_inline)) {
	  DO_PRAGMA(taffo pixels getPixel_b ANNOTATION_PIXEL)
	  float *pixels = ((float*)_pixels);
	  return pixels[y * (IMAGE_WIDTH(this) * 3) + x * 3 + 2];
	}
	
	inline void putPixel_r(int x, int y, float v)  __attribute((always_inline)) {
	  // function parameter pragma
	  DO_PRAGMA(taffo v putPixel_r ANNOTATION_PIXEL )

	  DO_PRAGMA(taffo pixels putPixel_r ANNOTATION_PIXEL)
	  float *pixels = ((float*)_pixels);
	  pixels[y * (IMAGE_WIDTH(this) * 3) + x * 3 + 0] = v;
	}

	inline void putPixel_g(int x, int y, float v)  __attribute((always_inline)) {
	  // function parameter pragma
      DO_PRAGMA(taffo v putPixel_g ANNOTATION_PIXEL)

      DO_PRAGMA(taffo pixels putPixel_g ANNOTATION_PIXEL)
	  float *pixels = ((float*)_pixels);
	  pixels[y * (IMAGE_WIDTH(this) * 3) + x * 3 + 1] = v;
	}

	inline void putPixel_b(int x, int y, float v)  __attribute((always_inline)) {
	  // function parameter pragma
	  DO_PRAGMA(taffo v putPixel_b ANNOTATION_PIXEL)

	  DO_PRAGMA(taffo pixels putPixel_b ANNOTATION_PIXEL)
	  float *pixels = ((float*)_pixels);
