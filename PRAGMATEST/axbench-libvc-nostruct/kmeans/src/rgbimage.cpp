...
#pragma taffo loadRgbImage "scalar()"
int loadRgbImage(const char* fileName, RgbImage* image, float scale) {
	//function parameters pragma
	#pragma taffo scale loadRgbImage "range 256.0 0.0"
	int c;
	int i;
	int j;
	char w[256];
...
#pragma taffo saveRgbImage "scalar()"
int saveRgbImage(RgbImage* image, const char* fileName, float scale) {
	//function parameters pragmas
	#pragma taffo scale saveRgbImage "range 0 256"
	int i;
	int j;
	FILE *fp;
	DO_PRAGMA(taffo pixels saveRgbImage ANNOTATION_RGBPIXEL)
...
void makeGrayscale(RgbImage* image) {
	int i;
	int j;
	DO_PRAGMA(taffo luminance makeGrayscale ANNOTATION_RGBPIXEL)
	float  luminance;
	DO_PRAGMA(taffo pixels makeGrayscale ANNOTATION_RGBPIXEL)
	float **pixels = (float **)image->pixels;

	DO_PRAGMA(taffo rC makeGrayscale ANNOTATION_RGBPIXEL)
	float rC = 0.30;
	DO_PRAGMA(taffo gC makeGrayscale ANNOTATION_RGBPIXEL)
	float gC = 0.59;
	DO_PRAGMA(taffo bC makeGrayscale ANNOTATION_RGBPIXEL)
	float bC = 0.11;
