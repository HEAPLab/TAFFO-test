...
#pragma taffo initClusters "scalar()"
int initClusters(Clusters* clusters, int k, float scale /* = 1 */) {
	//function parameters pragmas
	#pragma taffo scale initClusters "range 0 1"

	int i;
	DO_PRAGMA(taffo x initClusters ANNOTATION_CENTROID)
	float x;
	
	DO_PRAGMA(taffo centroids initClusters ANNOTATION_CENTROID)
...
void segmentImage(RgbImage* image, Clusters* clusters, int n) {
	...
	int x, y;
	int c;
	DO_PRAGMA(taffo pixels segmentImage ANNOTATION_RGBPIXEL)
	float **pixels = (float **)image->pixels;
	int **pixels2 = (int **)image->pixels;
	DO_PRAGMA(taffo centroids segmentImage ANNOTATION_CENTROID)
	...
	for (y = 0; y < IMAGE_H; y++) {
		for (x = 0; x < IMAGE_W; x++) {
			DO_PRAGMA(taffo r segmentImage "target:component " ANNOTATION_RGBPIXEL)
			DO_PRAGMA(taffo g segmentImage "target:component " ANNOTATION_RGBPIXEL)
			DO_PRAGMA(taffo b segmentImage "target:component " ANNOTATION_RGBPIXEL)
		  	float r, g, b;
