...
//used for pragmas that contain macros
...
#pragma taffo prices "scalar()"
fptype  *prices;
int numOptions;

int    * otype;
DO_PRAGMA(taffo sptprice sptprice_annotation)
fptype  *sptprice;
DO_PRAGMA(taffo strike strike_annotation)
fptype  *strike; // range(0.33,1.0)
#pragma taffo rate "scalar(range(0.0275,0.1) error(0))"
fptype  *rate;		//always
DO_PRAGMA(taffo volatility volatilty_annotation)
fptype  *volatility; // range(0.05,0.65)
#pragma taffo otime "scalar(range(0.1,1) error(0))"
fptype *otime;				// always
int numError = 0;
...
{
	#pragma taffo InputX CNDF "scalar()"
    int sign;

    #pragma taffo OutputX CNDF "scalar()"
    fptype  OutputX; //range(-106.998909,107.998909)
    #pragma taffo xInput CNDF "scalar()"
    fptype  xInput;
    #pragma taffo xNPrimeofX CNDF "scalar()"
    fptype  xNPrimeofX;
    #pragma taffo expValues CNDF "scalar()"
    fptype  expValues; //range(0.144059,6.941593)
    #pragma taffo xK2 CNDF "scalar()"
    fptype  xK2;
    #pragma taffo xK2_2 CNDF "scalar()"
    #pragma taffo xK2_3 CNDF "scalar()"
    fptype  xK2_2, xK2_3;
    #pragma taffo xK2_4 CNDF "scalar()"
    #pragma taffo xK2_5 CNDF "scalar()"
    fptype  xK2_4, xK2_5;
    #pragma taffo xLocal CNDF "scalar()"
    #pragma taffo xLocal_1 CNDF "scalar()"
    fptype  xLocal, xLocal_1;
    #pragma taffo xLocal_2 CNDF "scalar()"
    #pragma taffo xLocal_3 CNDF "scalar()"
			    			...
			    			fptype*  N2)
{
	...
	#pragma taffo sptprice BlkSchlsEqEuroNoDiv "scalar()"
	#pragma taffo strike BlkSchlsEqEuroNoDiv "scalar()"
	#pragma taffo rate BlkSchlsEqEuroNoDiv "scalar()"
	#pragma taffo volatility BlkSchlsEqEuroNoDiv "scalar()"
	#pragma taffo time BlkSchlsEqEuroNoDiv "scalar()"
	#pragma taffo timet BlkSchlsEqEuroNoDiv "scalar()"
	#pragma taffo N1 BlkSchlsEqEuroNoDiv "scalar(disabled)"
	#pragma taffo N2 BlkSchlsEqEuroNoDiv "scalar(disabled)"

    //printf("BlkSchlsEqEuroNoDiv %f %f %f %f %f %f\n", sptprice, strike, rate, volatility, time, timet);
    #pragma taffo OptionPrice BlkSchlsEqEuroNoDiv "scalar()"
    fptype OptionPrice;

    // local private working variables for the calculation
    //fptype xStockPrice;
    //fptype xStrikePrice;
    #pragma taffo xRiskFreeRate BlkSchlsEqEuroNoDiv "scalar()"
    fptype xRiskFreeRate;
    #pragma taffo xVolatility BlkSchlsEqEuroNoDiv "scalar()"
    fptype xVolatility;
    #pragma taffo xTime BlkSchlsEqEuroNoDiv "scalar()"
    fptype xTime;
    #pragma taffo xSqrtTime BlkSchlsEqEuroNoDiv "scalar()"
    fptype xSqrtTime;
    #pragma taffo logValues BlkSchlsEqEuroNoDiv "scalar()"
    fptype logValues;
    #pragma taffo xLogTerm BlkSchlsEqEuroNoDiv "scalar()"
    fptype xLogTerm;
    #pragma taffo xD1 BlkSchlsEqEuroNoDiv "scalar()"
    fptype xD1;
    #pragma taffo xD2 BlkSchlsEqEuroNoDiv "scalar()"
    fptype xD2;
    #pragma taffo xPowerTerm BlkSchlsEqEuroNoDiv "scalar()"
    fptype xPowerTerm;
    #pragma taffo xDen BlkSchlsEqEuroNoDiv "scalar()"
    fptype xDen;
    #pragma taffo d1 BlkSchlsEqEuroNoDiv "scalar()"
    fptype d1;
    #pragma taffo d2 BlkSchlsEqEuroNoDiv "scalar()"
    fptype d2;
    #pragma taffo FutureValueX BlkSchlsEqEuroNoDiv "scalar()"
    fptype FutureValueX;
    #pragma taffo NofXd1 BlkSchlsEqEuroNoDiv "scalar()"
    fptype NofXd1;
    #pragma taffo NofXd2 BlkSchlsEqEuroNoDiv "scalar()"
    fptype NofXd2;
    #pragma taffo NegNofXd1 BlkSchlsEqEuroNoDiv "scalar()"
    fptype NegNofXd1;
    #pragma taffo NegNofXd2 BlkSchlsEqEuroNoDiv "scalar()"
    fptype NegNofXd2;
...
int bs_thread(void *tid_ptr) {
    ...
    #pragma taffo price_orig bs_thread "target('price_orig') scalar(range(-128.071959,128.071959))"
    fptype price_orig;
    ...
    for (j=0; j<NUM_RUNS; j++) {
        for (i=start; i<end; i++) {
             ...
             */
        	#pragma taffo N1 bs_thread "scalar(disabled)"
        	#pragma taffo N2 bs_thread "scalar(disabled)"
            fptype N1,N2;
            #pragma taffo timet bs_thread "scalar(range(0,1) error(0))"
            float timet = 0;
/*
            double dataIn[6];
            double dataOut[1];
